...
def get_groceries_within_circle(df, circle):
    ...
    df_test_copy = df_test_copy[df_test_copy['in_circle'] == True]
    

    col = ['BUSINESS NAME','STREET ADDRESS']
    # Append the 3 cols  to their respective  list.
    business = []
    street = []
    for index, row in df_test_copy[col].iterrows():
        business.append(row[col[0]])
        street.append(row[col[1]])

    
    # Returns 3 values - Use indexing to get the value
    return business, street, df_test_copy
